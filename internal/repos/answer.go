...
func NewAnswerDB(db *database.DB) *AnswerDB {
	return &AnswerDB{db: db}
}

func (a *AnswerDB) Save(ctx context.Context, m *entities.Answer) error {
...
func (r *AnswerDB) scanOne(row pgx.Row) (*entities.Answer, error) {
	answer := entities.NewAnswer()
